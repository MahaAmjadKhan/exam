...
class CompleteResponderTest {
    ...
    @Order(1)
    @GradeValue(1)
    public void also_rule1() {

        String result;

        result = tryResponder("My mac has problems !");
        assertResponse("mac", result);

        result = tryResponder("It's an expensive mac");
        assertResponse("expensive", result);

        result = tryResponder("Is the mac a problem machine ?");
        assertNotResponse("mac", result);

        result = tryResponder("An expensive machine should work ?");
        assertNotResponse("expensive", result);

        result = tryResponder("Why don't you help me !");
        assertNotResponse("mac", result);
        assertNotResponse("expensive", result);

        result = tryResponder("Are you as expensive as this mac ?");
        assertNotResponse("mac", result);
        assertNotResponse("expensive", result);

        result = tryResponder("It is a slow and expensive mac ?");
        assertNotResponse("slow", result);
        assertNotResponse("expensive", result);
        assertResponse("mac", result);

        result = tryResponder("Can you get a less expensive mac ?");
        assertResponse("expensive", result);
        assertNotResponse("mac", result);
    }
    ...
    @Order(2)
    @GradeValue(1)
    public void also_rule2() {
        String result;

        result = tryResponder("My mac has problems !");
        assertResponse("mac", result);

        result = tryResponder("My mac is buggy !");
        assertResponse("buggy", result);

        result = tryResponder("A mac with buggy crappy performance and more besides !");
        assertResponse("performance", result);

        result = tryResponder("Linux would be an improvement over this buggy mac .");
        assertResponse("linux", result);

        result = tryResponder("I could choose linux over a bad performance ?");
        assertNotResponse("linux", result);
        assertNotResponse("performance", result);

        result = tryResponder("Or a mac over bad installation issues.");
        assertNotResponse("mac", result);

        result = tryResponder("Could mac memory or bad installation be making it slow ?");
        assertResponse("mac", result);
    }
    ...
    @Order(3)
    @GradeValue(1)
    public void also_rule3() {
        String result;
        String testWords[] = {"slow", "buggy", "installation" };
        HashMap<String,Integer> resultcounts = new HashMap<>();

        for (int rounds=0 ; rounds < 300; rounds++){
            tryResponder("My installation is buggy !");
            tryResponder("My machine is slow and buggy !");
            tryResponder("It's a slow installation");

            for(int i=0; i<6; i++) tryResponder("Spacing for rule 1");

            result = tryResponder("What a slow buggy installation");
            for(String s: testWords){
                resultcounts.put(s, resultcounts.getOrDefault(s, 0) + (matchResponse(s, result) ? 1 : 0 ));

            }
        }

        for(String s: testWords){
            int count = resultcounts.get(s);
            // System.out.println(count);
            if( count < 65 || count > 135)
                fail("Did not choose among three way tie evenly");
        }
    ...
    @Order(4)
    @GradeValue(1)
    public void also_rule4() {

        String result;

        tryResponder("My system installation of windows crashes all the time!");
        tryResponder("My system installation of windows crashes all the time!");
        tryResponder("My system installation of windows crashes all the time!");
        
        for(int i=0; i<6; i++) tryResponder("Spacing for rule 1");
        
        result = tryResponder("Help me with windows");
        assertResponse("windows", result);
        assertNotEquals(data.responseMap.get("windows"), result, "Sentences are not randomized");
        
        result = tryResponder("Help me with installation");
        assertResponse("installation", result);
        assertNotEquals(data.responseMap.get("installation"), result, "Sentences are not randomized");
        
        result = tryResponder("Help me with crashes");
        assertResponse("crashes", result);
        assertNotEquals(data.responseMap.get("crashes"), result, "Sentences are not randomized");
